...
{
    /// <summary>
    /// A <see cref="UITypeEditor"/> for selecting the <see cref="ActionDisplayID"/>.
    /// </summary>
    public class ActionDisplayIDEditor : UITypeEditor
    {
        ...
        public override object EditValue(ITypeDescriptorContext context, IServiceProvider provider, object value)
        {
            ...
            if (svc != null)
            {
                using (var editorForm = new ActionDisplayIDUITypeEditorForm(value))
                {
                    var pt = context.PropertyDescriptor.PropertyType;
                    if (svc.ShowDialog(editorForm) == DialogResult.OK)
                    {
                        if (pt == typeof(ActionDisplayID) || pt == typeof(ActionDisplayID?))
                            value = editorForm.SelectedItem.ID;
                        else if (pt == typeof(ActionDisplay))
